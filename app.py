...
YAMNET_MODEL_FILE_ID = '18FX4FDwmSUHZhbuQ_-GBDwwmzyFiL'  
...
print(f"[{datetime.now()}] Application startup: Checking for YAMNet model.")
# Downloading and extracting model if not already present
if not os.path.isdir(os.path.join(MODEL_DIR, 'yamnet-tensorflow2-yamnet-v1')):
    ...
    print(f"[{datetime.now()}] YAMNet model already present.")

print(f"[{datetime.now()}] Application start: Checking for class map.")
# Downloading and extracting class map if not already present
if not os.path.isfile(os.path.join(MODEL_DIR, 'yamnet_class_map.csv')):
    ...
    print(f"[{datetime.now()}] Class map already present.")


print(f"[{datetime.now()}] Application startup: Loading YAMNet model...")
yamnet_model_path = os.path.join(MODEL_DIR, 'yamnet-tensorflow2-yamnet-v1')
yamnet_model = tf.saved_model.load(yamnet_model_path)
print(f"[{datetime.now()}] Application startup: YAMNet model loaded.")

print(f"[{datetime.now()}] Application startup: Loading class names...")
...
def predict():
    try:
        ...
        if 'audio' not in request.files:
            print(f"[{datetime.now()}] No audio file in request")
            return jsonify({'error': 'No audio file provided'}), 400

        file = request.files['audio']
        print(f"[{datetime.now()}] File received: {file.filename}")
